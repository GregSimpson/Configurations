...
db_clean () {
	...
	log_both `docker run --rm --network realplay -v "$(pwd)/database/PostgreSQL/Migrations/sql":/flyway/sql -v "$(pwd)/database/PostgreSQL/Migrations/gjs_env":/flyway/conf flyway/flyway clean`
	popd 
}
...
db_fresh () {
	pushd ${LOCAL_REPO_ROOT}
	#log_both `eval pwd`
	log_both `docker run --rm --network realplay -v "$(pwd)/database/PostgreSQL/Migrations/sql":/flyway/gjs_env -v "$(pwd)/database/PostgreSQL/Migrations/gjs_env":/flyway/conf flyway/flyway baseline`
	popd 
	#log_both `eval pwd`
}
...
db_status () {
	pushd ${LOCAL_REPO_ROOT}
	log_both `docker run --rm --network realplay -v "$(pwd)/database/PostgreSQL/Migrations/sql":/flyway/sql -v "$(pwd)/database/PostgreSQL/Migrations/gjs_env":/flyway/conf flyway/flyway info`
	popd 
}

db_update () {
	pushd ${LOCAL_REPO_ROOT}
	log_both `docker run --rm --network realplay -v "$(pwd)/database/PostgreSQL/Migrations/sql":/flyway/sql -v "$(pwd)/database/PostgreSQL/Migrations/gjs_env":/flyway/conf flyway/flyway migrate`
	popd 
}

db_target () {
	pushd ${LOCAL_REPO_ROOT}
	log_both `docker run --rm --network realplay -v "$(pwd)/database/PostgreSQL/Migrations/sql":/flyway/sql -v "$(pwd)/database/PostgreSQL/Migrations/gjs_env":/flyway/conf flyway/flyway migrate -target=1.0.1`
	popd 
}
...
LOCAL_REPO_ROOT="/home/gsimpson/gjs/git_stuff/project-fischer"
# house keeping
clear
setup_logging
log_date
...
docker run --rm --network realplay -v "$(pwd)/database/PostgreSQL/Migrations/sql":/flyway/sql -v "$(pwd)/database/PostgreSQL/Migrations/gjs_env":/flyway/conf flyway/flyway info;
